...
import { httpPostFinanceOutcome } from "@/services/api/requests/finance-outcomes.requests";
...
const CreateFinanceOutcomesPage = () => {
  const { t } = useTranslation();

  const { isPending, mutateAsync } = useMutation({
    mutationFn: httpPostFinanceOutcome,
    onSuccess: () => {
      scrollToTop();
  ...
  return (
    <>
      <Helmet>
        <title>{t("Chiqim qo'shish")}</title>
      </Helmet>
      <Row gutter={[20, 20]}>
        <Col span={24}>
          <Flex align="center" justify="space-between">
            <PageTitle>{t("Chiqim qo'shish")}</PageTitle>
            <BackButton />
          </Flex>
        </Col>
        <Col span={24}>
...
export default CreateFinanceOutcomesPage;
