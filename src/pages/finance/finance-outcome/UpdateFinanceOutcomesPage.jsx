...
import { prepareFinanceOutcomeForEdit } from "@/services/api/prepare-data/finance-outcome";
import {
  httpGetFinanceOutcomeOne,
  httpUpdateFinanceOutcome,
} from "@/services/api/requests/finance-outcomes.requests";
...
const UpdateFinanceOutcomesPage = () => {
  const { t } = useTranslation();

  const { id } = useParams();

  const updateElementState = useQuery({
    queryKey: ["finance-outcome-one", id],
    queryFn: () => httpGetFinanceOutcomeOne(id),
    select: (response) => prepareFinanceOutcomeForEdit(response.data),
  });
  ...
  const { isPending, mutateAsync } = useMutation({
    mutationFn: httpUpdateFinanceOutcome,
  ...
  return (
    <>
      <Helmet>
        <title>{t("Chiqimni tahrirlash")}</title>
      </Helmet>
      <Row gutter={[20, 20]}>
        <Col span={24}>
          <Flex align="center" justify="space-between">
            <PageTitle>{t("Chiqimni tahrirlash")}</PageTitle>
            <BackButton />
          </Flex>
        </Col>
        <Col span={24}>
...
export default UpdateFinanceOutcomesPage;
