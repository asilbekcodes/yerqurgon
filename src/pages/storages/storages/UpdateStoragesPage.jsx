...
import { prepareStorageForEdit } from "@/services/api/prepare-data/storages";

const UpdateStoragesPage = () => {
  ...
  const updateElementState = useQuery({
    ...
    queryFn: () => httpGetStorageOne(id),
    select: (response) => prepareStorageForEdit(response.data),
  });
  ...
  return (
    <>
      <Helmet>
        <title>{t("Omborni tahrirlash")}</title>
      </Helmet>
      <Row gutter={[20, 20]}>
        <Col span={24}>
          <Flex align="center" justify="space-between">
            <PageTitle>{t("Omborni tahrirlash")}</PageTitle>
            <BackButton />
          </Flex>
        </Col>
        <Col span={24}>
...
export default UpdateStoragesPage;
