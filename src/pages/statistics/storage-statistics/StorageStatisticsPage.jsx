...
import { useStorageBreadcrumbItems } from "../breadcrumbs/useStorageBreadcrumb";
...
const StorageStatisticsPage = () => {
  const { t } = useTranslation();

  const BREADCRUMB_ITEMS = useStorageBreadcrumbItems();

  return (
    <>
      <Helmet>
        <title>{t("Ombor statistikasi")}</title>
      </Helmet>
      <Row gutter={[20, 20]}>
        <Col span={24}>
          <Flex align="center" justify="space-between">
            <PageTitle>{t("Ombor statistikasi")}</PageTitle>
            <Flex gap={"middle"}>
              <BackButton />
            </Flex>
          </Flex>
...
export default StorageStatisticsPage;
