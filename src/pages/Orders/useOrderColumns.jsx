...
export const useOrderColumns = (pagination, filters, setFilters, refetch) => {
  ...
  return [
        ...
        },
        {
          ...
          render: (_, record) => {
            return record.lead_products?.map((item, index) => (
              <div key={index}>{NumberToThousandFormat(get(item.product, "price", ""))}</div>
            ));
          },
        },
        {
    ...
    // },
    {
      ...
      key: "status",
      filters: [
        { text: t("Tayyorlanmoqda"), value: "PENDING" },
        { text: t("Yetkazildi"), value: "SUCCESS" },
      ],
      filteredValue: filters.status || null,
      render: (value) => {
        switch (value) {
          case "PENDING":
            return <Tag color={"yellow"}>{t("Tayyorlanmoqda")}</Tag>;
          case "SUCCESS":
            return <Tag color={"green"}>{t("Yetkazildi")}</Tag>;
        }
      },
    },
    {
