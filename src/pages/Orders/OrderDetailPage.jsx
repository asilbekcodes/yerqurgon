...
} from "./_components/add-payment-for-order/AddPaymentForOrder"
import { useDetailBreadcrumbItems } from "./breadcrumbs/useDetailBreadcrumb"
import { httpDeletePaymentOrder, httpGetOrderOne } from "@/services/api/requests/order.request"
                  ...
                  <>
                    <CustomTabs tabPosition={"top"}>
                            ...
                            <Card>
                              <Flex vertical gap={"large"}>
                                ...
                                <TitleAndIconText
                                  ...
                                  value={data?.lead_products?.map(p => p.product.name).join(", ") || ""}
                                  icon={<RiStackFill />}
                                />
                                <TitleAndIconText
                                ...
                                value={
                                  get(data, "status", "") === "PENDING" ? (
                                    <Tag color={"yellow"}>
                                      {t("Tayyorlanmoqda")} 
                                      {/* ({NumberToThousandFormat(get(data, "debt_balance", ""))}) */}
                                    </Tag>
                                  ) : (
                                    <Tag color={"green"}>{t("Yetkazilgan")}</Tag>
                                  )
                                }
                      ...
                      <Tabs.TabPane tab={t("Mahsulotlar")} key="2">
                        <Products data={get(data, "lead_products", [])} />
                      </Tabs.TabPane>
                      {
                        !isEmpty(
                          get(data, "services", []) && (
...
function Products({ data }) {
  ...
  const baseColumns = [
    ...
    },
    {
      ...
      render: (value) => {
        console.log(value);
        return <>{NumberToThousandFormat(get(value, "price", ""))}</> 
      },
    },
    {
...
function Payments({ data, refetch }) {
  ...
  const deleteMutate = useMutation({
    mutationFn: httpDeletePaymentOrder,
    onSuccess: () => {
