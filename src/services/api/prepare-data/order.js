...
export function prepareAddPaymentOrderDto(item) {
  return {
...
export function prepareOrderDto(item) {
  return {
    client: Number(get(item, "client", "")),
    user: Number(get(item, "user", "")),
    desc: get(item, "desc", ""),
    date: formatTimeForApi(get(item, "date", "")),
    ...
    lead_products: get(item, "products", []),
    services: get(item, "services", []),
  };
}

export function prepareOrderForEdit(item) {
